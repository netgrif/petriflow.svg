...
export class PetriflowTransitionPlaceArc extends RegularTransitionPlaceArc implements SelectableArc {
    ...
    cloneArc(start: NodeElement,  end: NodeElement): PetriflowTransitionPlaceArc {
        const newLinePoints = [];
        this.linePoints.forEach(point => newLinePoints.push(Object.assign({}, {
            x: point.x,
            y: point.y
        } as DOMPoint)));
        const cloned = new PetriflowTransitionPlaceArc(start, end, newLinePoints, this.multiplicity?.textContent);
